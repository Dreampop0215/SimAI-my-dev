...
void
QbbHelper::SetChannelType (std::string channelType,
                           std::string remoteChannelType)
{
  // 中文注释：将本地链路工厂切换到指定类型。
  m_channelFactory.SetTypeId (channelType);
  // 中文注释：若未显式指定 remote 类型，则保持与本地一致，避免自定义属性无法下发。
  if (remoteChannelType.empty ())
    {
      m_remoteChannelFactory.SetTypeId (channelType);
    }
  else
    {
      m_remoteChannelFactory.SetTypeId (remoteChannelType);
    }
}
