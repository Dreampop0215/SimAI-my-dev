...
void SendFlow(int src, int dst, uint64_t maxPacketCount,
  ...
  send_lat *= 1000;
  // 新增：physical 模式可选地引入“全局物理层发包开销”，保证同 topo/workload 下可观测差异。
  if (optical_node_impl == "physical" && optical_phy_force_tx_penalty_enable) {
    uint64_t packet_kb = (real_PacketCount + 1023) / 1024;
    uint64_t extra_tx_ns =
        static_cast<uint64_t>(optical_phy_force_tx_penalty_ns) +
        static_cast<uint64_t>(optical_phy_force_tx_per_kb_ns) * packet_kb;
    if (extra_tx_ns > 0) {
      send_lat += static_cast<int>(extra_tx_ns);
      NcclLog->writeLog(
          NcclLogLevel::DEBUG,
          "physical tx penalty enabled: src %d dst %d flow_id %d extra_tx_ns %lu",
          src, dst, flow_id, extra_tx_ns);
    }
  }
  flow_input.idx++;
