...
Sys::~Sys() {
  ...
  if (id == 0) {
    const double finished_percentage =
        streams_injected == 0
            ? 0.0
    ...
    std::cout << "*****" << std::endl
              ...
              << "Percentage of finished streams: " << finished_percentage
              << " %"
              << std::endl
              << "*****" << std::endl;
    if (streams_injected > 0 && streams_finished >= streams_injected) {
      std::cout.flush();
      exitSimLoop("Exiting");
    }
  }
  ...
  // 中文注释：退出判定改为检查 all_generators 全量槽位，避免 num_gpus 与实际节点集不一致时提前退出。
  for (size_t i = 0; i < all_generators.size(); ++i) {
...
Sys::Sys(
  ...
  // 中文注释：显式初始化 num_gpus，避免析构阶段读取未初始化值导致退出逻辑异常。
  this->num_gpus = _all_gpus.empty() ? 0 : _all_gpus[0];
...
void Sys::call_events() {
  ...
  for (auto& callable : event_queue[Sys::boostedTick()]) {
    try {
      Callable* target = std::get<0>(callable);
      EventType eventType = std::get<1>(callable);
      if (id == 0 && workload != nullptr &&
          workload->current_state == Workload::LoopState::Wait_For_Sim_Finish) {
        const uint64_t outstanding =
            (streams_injected >= streams_finished)
                ? (streams_injected - streams_finished)
                : 0;
        if (outstanding <= 2) {
          BaseStream* stream = dynamic_cast<BaseStream*>(target);
          if (stream != nullptr) {
            std::cout << "[Sys::tail-event] tick=" << Sys::boostedTick()
                      << ", event=" << static_cast<int>(eventType)
                      << ", stream_id=" << stream->stream_num
                      << ", q=" << stream->current_queue_id
                      << ", state=" << static_cast<int>(stream->state)
                      << ", outstanding=" << outstanding
                      << ", pending_events_before=" << pending_events
                      << std::endl;
          }
        }
      }
      pending_events--;
      target->call(eventType, std::get<2>(callable));
    } catch (...) {
...
void Sys::proceed_to_next_vnet_baseline(StreamBaseline* stream) {
  ...
  if (id == 0 && workload != nullptr &&
      workload->current_state == Workload::LoopState::Wait_For_Sim_Finish) {
    std::cout << "[Sys::tail-retire-enter] tick=" << Sys::boostedTick()
              << ", stream_id=" << stream->stream_num
              << ", q=" << stream->current_queue_id
              << ", steps=" << stream->steps_finished
  ...
  if (stream->phases_to_go.size() == 0) {
    ...
    if (id == 0 && workload != nullptr &&
        workload->current_state == Workload::LoopState::Wait_For_Sim_Finish) {
      std::cout << "[Sys::tail-retire-final] tick=" << Sys::boostedTick()
                << ", stream_id=" << stream->stream_num
                << ", previous_vnet=" << previous_vnet
                << std::endl;
    }
    delete stream;
    return;
