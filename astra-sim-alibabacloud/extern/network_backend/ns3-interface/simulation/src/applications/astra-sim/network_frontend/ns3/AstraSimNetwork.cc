...
int main(int argc, char *argv[]) {
  ...
  // 中文注释：统一将框架日志写入 ./log 目录，便于每次运行后固定路径排查。
  std::filesystem::create_directories("./log");
  MockNcclLog::set_log_name("./log/SimAI.log");
  MockNcclLog* NcclLog = MockNcclLog::getInstance();
  ...
  // 中文注释：AstraSim system 的逻辑节点应仅为 GPU，不能把 NVSwitch/光模块计入 workload rank。
  int gpu_num = node_num - nvswitch_num - switch_num - optical_module_num;
  ...
  for (int j = 0; j < nodes_num; j++) {
    ...
    // 中文注释：显式使用真实 GPU 数，避免因中间节点计入导致通信组构建错乱。
