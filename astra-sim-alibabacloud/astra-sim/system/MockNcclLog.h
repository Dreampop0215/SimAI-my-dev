...
class MockNcclLog {
 ...
 public:
  ...
  static void set_log_name(std::string log_name){
    // 中文注释：支持传入相对/绝对路径（如 ./log/SimAI.log），否则保持历史行为写入 LOG_PATH。
    if (log_name.rfind("./", 0) == 0 || log_name.rfind("../", 0) == 0 ||
        (!log_name.empty() && log_name[0] == '/')) {
      LogName = log_name;
    } else {
      LogName = LOG_PATH + log_name;
    }
    // 中文注释：确保日志目录存在，避免因目录不存在导致日志文件无法创建。
    std::error_code ec;
    auto parent = std::filesystem::path(LogName).parent_path();
    if (!parent.empty()) {
